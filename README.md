# FlowGuard Interactive Demo (demo.flowguard.ng)

A branded wrapper page that hosts the 3D estate render in a full-screen iframe,
plus the render itself. Deploys as its own Netlify site on the `demo.flowguard.ng`
...
- Netlify site → **Domain settings → Add custom domain → `demo.flowguard.ng`**.
...
`https://demo.flowguard.ng`, so once the subdomain is live the flow works.
